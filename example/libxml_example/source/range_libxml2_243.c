...
range (char * program_path, char * char_seq_path) {
	...
	fprintf(stderr, "lastminimized: %s, %ld\n", char_seq_path, seq_len);
	...
	for (int rs = seq_len-1; rs >=1; rs--) {
		...
		for (int begin = 0; begin <= seq_len - rs; begin++) {
			...
			FILE * out_fp = fopen(f_name, "w");
			FILE * in_fp = fopen(char_seq_path, "r");
			...
			int e_code = test_buffer_overflow(program_path, f_name);
                        if (e_code == 1) {
				return range(program_path, f_name);
			}
